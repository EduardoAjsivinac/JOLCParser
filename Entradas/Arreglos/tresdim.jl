...
function imprimirScore(array,mapas,enemigos,jugadores)
    for i in 1:length(array)
        for j in 1:length(array[i])
            for k in 1:length(array[i][j])
                println(mapas[i] * "          " * enemigos[j] * "          " * jugadores[k] * "          " * string(array[i][j][k]));
...
imprimirScore(score,mapas,enemigos,jugadores);